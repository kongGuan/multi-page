...
const StringUtils = {
  ...
  isBlank: function (input) {
    return typeof input === "undefined" || input === null || /^\s*$/.test(input);
  },
  trim: function (input) {
    if (this.isEmpty(input)) return ''
  ...
  joinChar: function (input, char, index) {
    if (this.isBlank(input)) return ""
    var re = new RegExp("(\\w{" + index + "})(?=\\w)", "g"); //注意，反斜杠需要转义
    return input.replace(re, "$1" + char);
  }
};
